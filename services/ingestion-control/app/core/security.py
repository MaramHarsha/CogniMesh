...
RUN_ROLES = {"platform_admin", "workspace_admin", "data_engineer", "service_account"}
WRITE_ROLES = {"platform_admin", "workspace_admin", "data_engineer", "service_account"}
...
def get_request_context(
    ...
    if not get_settings().allow_dev_auth:
        raise HTTPException(status_code=503, detail="Ingestion auth provider is not configured")
    ...
    return RequestContext(
        ...
        roles=roles,
        purpose=x_cognimesh_purpose or "raw_ingestion",
...
def authorize(context: RequestContext, action: str) -> None:
    ...
    if not set(context.roles).intersection(allowed_roles):
        raise HTTPException(status_code=403, detail=f"Roles {sorted(context.roles)} cannot {action} ingestion assets")
