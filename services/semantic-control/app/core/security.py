...
def get_request_context(
    ...
    if not get_settings().allow_dev_auth:
        raise HTTPException(status_code=503, detail="Semantic auth provider is not configured")
    ...
    return RequestContext(
        ...
        roles=roles,
        purpose=x_cognimesh_purpose or "semantic_modeling",
...
def authorize(context: RequestContext, action: str) -> None:
    ...
    if not set(context.roles).intersection(allowed_roles):
        raise HTTPException(status_code=403, detail=f"Roles {sorted(context.roles)} cannot {action} semantic assets")
