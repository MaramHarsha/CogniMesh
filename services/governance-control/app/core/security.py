...
WRITE_ROLES = {"platform_admin", "workspace_admin", "data_steward", "data_engineer", "service_account"}
APPROVER_ROLES = {"platform_admin", "workspace_admin", "data_steward"}
...
def get_request_context(
    ...
    return RequestContext(
        ...
        roles=roles,
        purpose=x_cognimesh_purpose or "governance_operations",
...
def authorize(context: RequestContext, action: str) -> None:
    ...
    if not set(context.roles).intersection(allowed_roles):
        raise HTTPException(
            status_code=403,
            detail=f"Roles {sorted(context.roles)} cannot {action} governance assets",
        )
