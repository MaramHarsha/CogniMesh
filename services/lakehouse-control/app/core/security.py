...
ADMIN_ROLES = {"platform_admin", "workspace_admin", "data_engineer"}
...
def get_request_context(
    ...
    if not get_settings().allow_dev_auth:
        raise HTTPException(status_code=503, detail="Lakehouse auth provider is not configured")
...
def authorize(context: RequestContext, action: str) -> None:
    ...
    elif action in {"admin", "maintenance", "merge"}:
        allowed_roles = ADMIN_ROLES
    else:
        allowed_roles = WRITE_ROLES

    if not set(context.roles).intersection(allowed_roles):
        raise HTTPException(status_code=403, detail=f"Roles {sorted(context.roles)} cannot {action} lakehouse assets")
