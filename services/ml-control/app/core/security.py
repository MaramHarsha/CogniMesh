...
APPROVER_ROLES = {"platform_admin", "workspace_admin", "data_steward", "ml_engineer"}
...
def get_request_context(
    ...
    return RequestContext(
        ...
        roles=roles,
        purpose=x_cognimesh_purpose or "ml_operations",
...
def authorize(context: RequestContext, action: str) -> None:
    ...
    if not set(context.roles).intersection(allowed_roles):
        raise HTTPException(
            status_code=403,
            detail=f"Roles {sorted(context.roles)} cannot {action} ML assets",
        )
