...
WRITE_ROLES = {"platform_admin", "workspace_admin", "data_engineer", "analyst", "service_account"}
...
def get_request_context(
    ...
    return RequestContext(
        ...
        roles=roles,
        purpose=x_cognimesh_purpose or "planning_operations",
...
def authorize(context: RequestContext, action: str) -> None:
    ...
    if not set(context.roles).intersection(allowed_roles):
        raise HTTPException(
            status_code=403,
            detail=f"Roles {sorted(context.roles)} cannot {action} planning assets",
        )
